...
class Test(unittest.TestCase):
    ...
    def testFoss_Fcase004(self):
        ...
        try:
            ...
            Login(self,"http://192.168.10.50/stl-web/login/index.action","009593","qqqqqq","上海车队")
            Navigation(self,"约车管理-外请车约车")
            DealAskOutCarAppoint(self,self.GetReturnValue(46,9),"渝BC8202","100")
        except:
            self.boolean=False
            raise RuntimeError
        finally:
            if self.boolean==True:
                self.WriteRes(47, 8, "True!")
            else:
                self.WriteRes(47, 8, 'FALSE!')
            if GetReturn !="":
                self.WriteRes(47,9,GetReturn)
            ...
            try:
                ...
                pass
            Path=Path+"\\Fcase004"
            Local = Path+"\\Scase002"+"-"+datetime.datetime.now().strftime('%Y_%m_%d %H_%M_%S')+".jpg"
            try:
                os.mkdir(Path)
            except:
                pass
