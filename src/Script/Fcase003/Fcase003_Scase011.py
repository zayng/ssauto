...
class Test(unittest.TestCase):
    ...
    def testFoss_Fcase003(self):
        GetReturn=""
        try:
            
            Login(self,"http://192.168.10.50/stl-web/login/index.action","044391","qqqqqq","上海虹口营业部")
            Navigation(self,"车辆管理-出发到达确认")
            ConfirmArrive(self,self.GetReturnValue(29,9),"2013-05-20 00:00:00","2013-06-14 23:59:59")
        except:
            self.boolean=False
            raise RuntimeError
        finally:
            if self.boolean==True:
                self.WriteRes(39, 8, "True!")
            else:
                self.WriteRes(39, 8, 'FALSE!')
            if GetReturn !="":
                self.WriteRes(39,9,GetReturn)
            ...
            try:
                ...
                pass
            Path=Path+"\\Fcase003"
            Local = Path+"\\Scase011"+"-"+datetime.datetime.now().strftime('%Y_%m_%d %H_%M_%S')+".jpg"
            try:
                os.mkdir(Path)
            except:
                pass
