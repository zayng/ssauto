...
class Test(unittest.TestCase):
    ...
    def testFoss_Fcase003(self):
        ...
        try:
            ...
            SettlementGoods(self,"999999926","天天","110101198812012773","0","0","qqqqqq")
        except:
            self.boolean=False
            raise RuntimeError
        finally:
            if self.boolean==True:
                self.WriteRes(44, 8, "True!")
            else:
                self.WriteRes(44, 8, 'FALSE!')
            if GetReturn !="":
                self.WriteRes(44,9,GetReturn)
            ...
            try:
                ...
                pass
            Path=Path+"\\Fcase003"
            Local = Path+"\\Scase016"+"-"+datetime.datetime.now().strftime('%Y_%m_%d %H_%M_%S')+".jpg"
            try:
                os.mkdir(Path)
            except:
                pass
