...
class Test(unittest.TestCase):
    ...
    def testFoss_Fcase003(self):
        ...
        try:
            ...
            GetReturn=NewUnloadTask(self,"",self.GetReturnValue(29,9),"熊子杰")
        except:
            self.boolean=False
            raise RuntimeError
        finally:
            if self.boolean==True:
                self.WriteRes(41, 8, "True!")
            else:
                self.WriteRes(41, 8, 'FALSE!')
            if GetReturn !="":
                self.WriteRes(41,9,GetReturn)
            ...
            try:
                ...
                pass
            Path=Path+"\\Fcase003"
            Local = Path+"\\Scase013"+"-"+datetime.datetime.now().strftime('%Y_%m_%d %H_%M_%S')+".jpg"
            try:
                os.mkdir(Path)
            except:
                pass
