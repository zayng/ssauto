...
class Test(unittest.TestCase):
    ...
    def testFoss_Fcase003(self):
        ...
        try:
            ...
            ConfirmUnloadTask(self,self.GetReturnValue(41,9))
        except:
            self.boolean=False
            raise RuntimeError
        finally:
            if self.boolean==True:
                self.WriteRes(42, 8, "True!")
            else:
                self.WriteRes(42, 8, 'FALSE!')
            if GetReturn !="":
                self.WriteRes(42,9,GetReturn)
            ...
            try:
                ...
                pass
            Path=Path+"\\Fcase003"
            Local = Path+"\\Scase014"+"-"+datetime.datetime.now().strftime('%Y_%m_%d %H_%M_%S')+".jpg"
            try:
                os.mkdir(Path)
            except:
                pass
