...
class Test(unittest.TestCase):
    ...
    def testFoss_Fcase001(self):
        ...
        try:
            ...
            SettlementGoods(self,"999999935","天天","110101198812012773","0","0","qqqqqq")
        except:
            self.boolean=False
            raise RuntimeError
        finally:
            if self.boolean==True:
                self.WriteRes(18, 8, "True!")
            else:
                self.WriteRes(18, 8, 'FALSE!')
            if GetReturn !="":
                self.WriteRes(18,9,GetReturn)
            ...
            try:
                ...
                pass
            Path=Path+"\\Fcase001"
            Local = Path+"\\Scase018"+"-"+datetime.datetime.now().strftime('%Y_%m_%d %H_%M_%S')+".jpg"
            try:
                os.mkdir(Path)
            except:
                pass
