...
class Test(unittest.TestCase):
    ...
    def testFoss_Fcase_001(self):
        try:
            
            #Login(self,"http://192.168.10.50/stl-web/login/index.action","074932","qqqqqq","上海青浦重固营业部")
            #Navigation(self,"装车管理-交接单管理")
            GetReturn="test1"#AddEir(self,"短配交接单","上海转运场","沪D02018","刘得松","931987392","2013-05-22 09:37:47")
        except:
            self.boolean=False
            raise RuntimeError
        finally:
            ...
            try:
                ...
                pass
            Path=Path+"\\"+"Fcase_001"
            Local = Path+"\\"+"Scase_001"+"-"+datetime.datetime.now().strftime('%Y_%m_%d %H_%M_%S')+".jpg"
            try:
                os.mkdir(Path)
            except:
                pass
