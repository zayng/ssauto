...
class Test(unittest.TestCase):
    ...
    def testFoss_Fcase001(self):
        ...
        try:
            ...
            Login(self,"http://192.168.10.50/stl-web/login/index.action","001321","qqqqqq","上海闵行区浦江镇营业部")
            Navigation(self,"调度管理-月台调度")
            SearchPlatform(self,"001")
            #GetReturn=AddEir(self,"短配交接单","上海转运场","粤A71418","刘洋洋","999999935","2013-06-14 09:37:47")
        except:
            self.boolean=False
            raise RuntimeError
        finally:
            if self.boolean==True:
                self.WriteRes(1, 8, "True!")
            else:
                self.WriteRes(1, 8, 'FALSE!')
            if GetReturn !="":
                self.WriteRes(1,9,GetReturn)
            ...
            try:
                ...
                pass
            Path=Path+"\\Fcase001"
            Local = Path+"\\Scase001"+"-"+datetime.datetime.now().strftime('%Y_%m_%d %H_%M_%S')+".jpg"
            try:
                os.mkdir(Path)
            except:
                pass
