...
class Test(unittest.TestCase):
    ...
    def testFoss_Fcase004(self):
        ...
        try:
            ...
            GetReturn=AddEir(self,"短配交接单","上海转运场","渝BC8202","李琦","999999298","2013-06-14 09:37:47")
        except:
            self.boolean=False
            raise RuntimeError
        finally:
            if self.boolean==True:
                self.WriteRes(49, 8, "True!")
            else:
                self.WriteRes(49, 8, 'FALSE!')
            if GetReturn !="":
                self.WriteRes(49,9,GetReturn)
            ...
            try:
                ...
                pass
            Path=Path+"\\Fcase004"
            Local = Path+"\\Scase004"+"-"+datetime.datetime.now().strftime('%Y_%m_%d %H_%M_%S')+".jpg"
            try:
                os.mkdir(Path)
            except:
                pass
