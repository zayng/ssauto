...
class Test(unittest.TestCase):
    ...
    def testFoss_Fcase004(self):
        ...
        try:
            ...
            ConfirmArrive(self,self.GetReturnValue(49,9),"2013-05-20 00:00:00","2013-06-14 23:59:59")
        except:
            self.boolean=False
            raise RuntimeError
        finally:
            if self.boolean==True:
                self.WriteRes(52, 8, "True!")
            else:
                self.WriteRes(52, 8, 'FALSE!')
            if GetReturn !="":
                self.WriteRes(52,9,GetReturn)
            ...
            try:
                ...
                pass
            Path=Path+"\\Fcase004"
            Local = Path+"\\Scase007"+"-"+datetime.datetime.now().strftime('%Y_%m_%d %H_%M_%S')+".jpg"
            try:
                os.mkdir(Path)
            except:
                pass
