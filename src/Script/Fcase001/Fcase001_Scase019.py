...
class Test(unittest.TestCase):
    ...
    def testFoss_Fcase001(self):
        ...
        try:
            ...
            SignSelf(self,"999999935")
        except:
            self.boolean=False
            raise RuntimeError
        finally:
            if self.boolean==True:
                self.WriteRes(19, 8, "True!")
            else:
                self.WriteRes(19, 8, 'FALSE!')
            if GetReturn !="":
                self.WriteRes(19,9,GetReturn)
            ...
            try:
                ...
                pass
            Path=Path+"\\Fcase001"
            Local = Path+"\\Scase019"+"-"+datetime.datetime.now().strftime('%Y_%m_%d %H_%M_%S')+".jpg"
            try:
                os.mkdir(Path)
            except:
                pass
