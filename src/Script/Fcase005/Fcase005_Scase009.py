...
class Test(unittest.TestCase):
    ...
    def testFoss_Fcase005(self):
        ...
        try:
            ...
            Navigation(self,"外发管理-偏线管理-查询偏线外发单")
            NewFarOffWaybill(self,"999999927","666666665","王成法","111")
        except:
            self.boolean=False
            raise RuntimeError
        finally:
            if self.boolean==True:
                self.WriteRes(71, 8, "True!")
            else:
                self.WriteRes(71, 8, 'FALSE!')
            if GetReturn !="":
                self.WriteRes(71,9,GetReturn)
            ...
            try:
                ...
                pass
            Path=Path+"\\Fcase005"
            Local = Path+"\\Scase009"+"-"+datetime.datetime.now().strftime('%Y_%m_%d %H_%M_%S')+".jpg"
            try:
                os.mkdir(Path)
            except:
                pass
