...
class Test(unittest.TestCase):
    ...
    def testFoss_Fcase001(self):
        ...
        try:
            ...
            Login(self,"http://192.168.10.50/stl-web/login/index.action","001235","qqqqqq","廊坊转运场")
            Navigation(self,"车辆管理-出发到达确认")
            ConfirmArrive(self,self.GetReturnValue(9,9),"2013-05-20 00:00:00","2013-06-14 23:59:59")
        except:
            self.boolean=False
            raise RuntimeError
        finally:
            if self.boolean==True:
                self.WriteRes(13, 8, "True!")
            else:
                self.WriteRes(13, 8, 'FALSE!')
            if GetReturn !="":
                self.WriteRes(13,9,GetReturn)
