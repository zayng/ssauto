...
class Test(unittest.TestCase):
    ...
    def testFoss_Fcase004(self):
        ...
        try:
            ...
            LoadSeal(self,self.GetReturnValue(49,9),"33153288")
        except:
            self.boolean=False
            raise RuntimeError
        finally:
            if self.boolean==True:
                self.WriteRes(50, 8, "True!")
            else:
                self.WriteRes(50, 8, 'FALSE!')
            if GetReturn !="":
                self.WriteRes(50,9,GetReturn)
            ...
            try:
                ...
                pass
            Path=Path+"\\Fcase004"
            Local = Path+"\\Scase005"+"-"+datetime.datetime.now().strftime('%Y_%m_%d %H_%M_%S')+".jpg"
            try:
                os.mkdir(Path)
            except:
                pass
